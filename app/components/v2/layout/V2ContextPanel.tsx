...
const CHART_DATES = ["21 May", "23 May", "25 May", "27 May"];
...
const CARD_HEADER = "flex items-center justify-between border-b border-white/[0.06] px-4 py-2";
...
export default function V2ContextPanel({ kpi, ctx }: { kpi: MockKpi; ctx: MockContext }) {
  ...
  return (
    <aside className="flex w-[260px] shrink-0 flex-col gap-2 overflow-y-auto">

      {/* ── Block 1: Neden Bu Sıralama? ── */}
      <div className={CARD}>
        ...
        <ul className="space-y-1.5 px-4 py-2">
          {rankingReasons.map((reason, i) => (
            <li key={i} className="flex items-start gap-2">
              <CheckIcon />
              <span className="text-[10.5px] leading-snug text-zinc-300">{reason}</span>
            </li>
          ))}
        </ul>
      </div>

      {/* ── Block 2: Kuyruk Gelir Özeti ── */}
      <div className={CARD}>
        <div className={CARD_HEADER}>
          <h3 className={SECTION_TITLE}>Kuyruk Gelir Özeti</h3>
          <button
            type="button"
            className="flex items-center gap-1 rounded-md border border-white/[0.08] bg-white/[0.04] px-2 py-0.5 text-[9px] font-semibold text-zinc-400 hover:bg-white/[0.07]"
          >
            7 Günlük
        ...
        {/* MRR + ARR metrics */}
        <div className="flex gap-3 border-b border-white/[0.04] px-4 py-1.5">
          <div className="flex-1">
            <div className="mb-1 flex items-center gap-1.5">
              <span className="h-1.5 w-3 rounded-full bg-blue-400" />
              <span className="text-[9px] text-zinc-500">Ağırlıklı MRR</span>
            </div>
            <p className="text-[13px] font-bold tabular-nums leading-none text-zinc-100">
              {fmtCurrencyTR(kpi.totalWeightedMrr)}
            </p>
            <p className="mt-0.5 text-[9px] text-zinc-600">/ay</p>
          </div>
          <div className="flex-1">
            <div className="mb-1 flex items-center gap-1.5">
            ...
            <p className="mt-0.5 text-[9px] text-zinc-600">/yıl</p>
          </div>
        </div>
        ...
        <div className="px-4 pb-2 pt-2">
          ...
          >
            ...
            />
            {CHART_X.map((x, i) => (
              <circle key={`mrr-${i}`} cx={x} cy={CHART_MRR_Y[i]} r="2.5" fill="#3b82f6" />
            ))}
            {CHART_X.map((x, i) => (
              <circle key={`arr-${i}`} cx={x} cy={CHART_ARR_Y[i]} r="2" fill="#10b981" />
            ))}
          </svg>
          <div className="mt-1 flex justify-between">
            {CHART_DATES.map((d) => (
              <span key={d} className="text-[8px] text-zinc-700">{d}</span>
            ))}
          </div>
        </div>
      </div>

      {/* ── Block 3: Bugünün Önerilen Aksiyon Planınız ── */}
      <div className={CARD}>
        <div className={CARD_HEADER}>
              ...
              >
                <div className="flex items-start justify-between gap-1">
                  <div className="min-w-0">
                    <p className={`text-[16px] font-bold tabular-nums leading-none ${c.count}`}>
                      {tile.count}
                    </p>
                    <p className="mt-1 text-[9px] font-semibold leading-tight text-zinc-200">
                      {tile.label}
                    </p>
                    <p className="mt-0.5 text-[8px] leading-tight text-zinc-600">{tile.sub}</p>
                  </div>
                  <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md ${c.bg}`}>
                    <svg
                      viewBox="0 0 16 16"
                      fill="none"
